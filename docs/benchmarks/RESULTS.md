...
`per_task.csv` and `pooled_gate.csv` include SHA-256 pointers to the exact source files. `profile_manifest.json` records the source and artifact hashes for every bundled profile. Raw benchmark prompts, completions, and model weights are intentionally excluded from this product repository.
